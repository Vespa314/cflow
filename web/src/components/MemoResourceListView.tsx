...
const MemoResourceListView = ({ resourceList = [] }: { resourceList: Resource[] }) => {
  ...
  const handleImageClick = (imgUrl: string) => {
    const imgUrls = mediaResources
      ...
      .map((resource) => construct_url(getResourceUrl(resource), false));
    const index = imgUrls.findIndex((url) => url.split("?")[0] === imgUrl);
    showPreviewImageDialog(imgUrls, index);
  };

  const construct_url = (url: string, thumbnail?: boolean) => {
    return `${url}${thumbnail ? "?thumbnail=1" : ""}`;
  }
    ...
    if (type === "image/*") {
      return (
        <img
          ...
          src={resource.externalLink ? url : construct_url(url, thumbnail)}
