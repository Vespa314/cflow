...
export function updateResourceName(resource_id: number, filename: string) {
  return axios.patch<Resource>(`/api/v1/resource/${resource_id}`, {"filename": filename});
}

export function getTagCnt(tagFind?: TagFind) {
  const queryList = [];
  if (tagFind?.creatorUsername) {
    queryList.push(`creatorUsername=${tagFind.creatorUsername}`);
  }
  return axios.get<Record<string, number>>(`/api/v1/tag_cnt?${queryList.join("&")}`);
}
