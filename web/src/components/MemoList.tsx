  ...
  const shownMemos = (
    showMemoFilter
      ? memos.filter((memo) => {
          ...
          if (textQuery) {
            const queryWords = textQuery.toLowerCase().trim().split(/\s+/);
            for (let word of queryWords) {
              if (!word.startsWith('!') && !memo.content.toLowerCase().includes(word)) {
                shouldShow = false;
                break;
              }
              if (word.startsWith('!') && word.length > 1 && memo.content.toLowerCase().includes(word.slice(1))) {
                shouldShow = false;
                break;
              }
            }
