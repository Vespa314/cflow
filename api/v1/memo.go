...
func webhook(user_id, momo_id int, method string) {
	url := "http://xxxxxxxxxxxxxxx.com"
	payload := []byte(`{"memo_id": ` + strconv.Itoa(momo_id) + `, "method": "` + method + `", "user_id": ` + strconv.Itoa(user_id) + `}`)
