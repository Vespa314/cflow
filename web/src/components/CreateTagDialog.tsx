...
const CreateTagDialog: React.FC<Props> = (props: Props) => {
  ...
  return (
    <>
      ...
      <div className="dialog-content-container max-w-3xl">
        <Input
          className="mb-2"
          size="md"
          ...
          <>
            ...
            <div className="w-full flex flex-row justify-start items-start flex-wrap">
              {Array.from(tagNameList)
                ...
                .map((tag) => (
                  <span
                    className="text-sm mr-2 mt-1 font-mono cursor-pointer truncate dark:text-gray-300 hover:opacity-60 hover:line-through"
                    key={tag}
                    onClick={() => handleDeleteTag(tag)}
                  >
                    {
                    tagCounts[tag] > 10 ?
                    <span className="text-green-600">#{tag}({tagCounts[tag]})</span>  : tagCounts[tag] > 5 ?
                    <span className="text-blue-600">#{tag}({tagCounts[tag]})</span>   : tagCounts[tag] > 0 ?
                    <span className="text-gray-600">#{tag}({tagCounts[tag]})</span> : <span className="text-red-600">#{tag}</span>}
                  </span>
                ))}
            </div>
          </>
